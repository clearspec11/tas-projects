<script lang="ts">
	import { projects, selectedProject, filterCategory, filterStatus, filterFunding, searchQuery, timelineRange } from '$lib/stores';
	import { STATUS_CONFIG, CATEGORY_CONFIG, FUNDING_CONFIG, type Project, type ProjectCategory, type ProjectStatus, type FundingType } from '$lib/types';
			...
			>
				<div class="flex items-start justify-between gap-2">
					<div class="flex-1 min-w-0">
						...
						<div class="text-xs text-[var(--color-text-muted)] mt-0.5">{project.location_name}</div>
					</div>
					<div class="flex flex-col items-end gap-0.5">
						<span class="text-[10px] px-2 py-0.5 rounded-full font-medium whitespace-nowrap {cfg.bgClass}">
							{cfg.label}
						</span>
						{#if project.funding_type !== 'public'}
							<span class="text-[9px] px-1.5 py-0.5 rounded-full font-medium whitespace-nowrap bg-amber-500/15 text-amber-400">
								{fundCfg.shortLabel}
							</span>
						{/if}
					</div>
